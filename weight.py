...
## הוספנו 2 משתנים גלובליים 
global clientname, CONNECTED ,weight_, cups_
CONNECTED = False
r=random.randrange(1,10000000)
clientname="TAMI"+str(r) # החלפנו את השם של הקליינט
DHT_topic = 'pr/Tami5/5976397/sts' # החלפנו פה משהו''''''''''''''פ
update_rate = 5000 # in msec
weight_,cups_ = 1000,80 # מגדירים את הכמות ההתחלתית של הנתונים שאיתם נעבוד
...
class ConnectionDock(QDockWidget):
    ...
    def __init__(self,mc):
        ...
        ## השורות האלו מאפסות בהתחלה את הנתונים בחיישנים בתיבת טקסט
        ...
        # החלפנו את השמות של החיישנים
        formLayot.addRow("Weight",self.Weight)
        formLayot.addRow("Cups",self.Cups)
...
class MainWindow(QMainWindow):
    ...
    def update_data(self):
        global weight_,cups_
        print('Next update')
        
        # Changed here (הוספנו 2 משתנים משקל + כמות כוסות, כל עוד כמות הכוסות או כמות הסודה היא מעל 0, אז זה יפחית) 
        if weight_ > 0:
            weight_= weight_ - random.randrange(8,12)
        elif weight_ < 0:
            weight_ = 0
        ...
        # כאן זה שולח הודעות לגבי הסטטוס של הנתונים
        current_data='Weight: '+str(weight_)+' Cups: '+str(cups_)
        self.connectionDock.Weight.setText(str(weight_))
        self.connectionDock.Cups.setText(str(cups_))
